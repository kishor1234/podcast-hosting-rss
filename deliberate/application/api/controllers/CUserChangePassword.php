...
class CUserChangePassword extends CAaskController {
    ...
    function changePassword($postData) {
        $data = null;
        try {
            $sql = $this->ask_mysqli->select("user", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("email" => $this->encript->decTxt($postData["etoken"]), "id" => $this->encript->decTxt($postData["token"])), "AND");
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            if ($row = $result->fetch_assoc()) {
                if (strcmp($row["email"], $this->encript->decTxt($postData["etoken"])) == 0) {
                    $sql = $this->ask_mysqli->update(array("password" => password_hash($postData["pass"], PASSWORD_DEFAULT)), "user") . $this->ask_mysqli->where(array("email" => $this->encript->decTxt($postData["etoken"]), "id" => $this->encript->decTxt($postData["token"])), "AND");
                    if ($this->adminDB[$_SESSION["db_1"]]->query($sql)) {
                        ...
                        $this->mailObject->sendmailWithoutAttachment($email, noreplayid, company, $content, "Security alert for changing your password.", "");
                        ob_end_clean();
                        ob_start();

                        $data = array("message" => "password change successfully login to continue.", "status" => 1, "email" => $row["email"], "mail" => $mail);
                    } else {
                        $data = null;
                    }
                } else {
